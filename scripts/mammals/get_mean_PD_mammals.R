# Purpose ----
# Get the mean phylogenetic diversity (PD) with 100 posterior mammal trees
# 
# Date: 2022/11/02
wd <- '/Users/imac2016/Dropbox/hector_working/richness_pd_dr'
setwd(wd)
sink(file="output/get_meanPD_output_MAMMALS.txt", 
     append=F, split=TRUE)
...
# Phylogeny ----
postrees_file <- 'objects/tree_post_100_MAMMALS.rds'
postrees <- readRDS(postrees_file)
print("Posterior trees loaded")

# Import EPM object
mam_EPM <- readRDS('objects/EPM_mam.rds')
print("mam_EPM object loaded")
...
for (i in 1:length(postrees)){
     ...
     tree <- postrees[[i]]
     mam_EPM <- addPhylo(mam_EPM, tree, replace = TRUE)
     print("Phylogeny added to EPM object")

     print("running gridMetrics() to get PD...") 
     epm_pd <- gridMetrics(mam_EPM, metric = 'pd')
...
# Save pd_mean object ----
saveRDS(pd_mean, 'objects/pd_mean_MAMMALS.rds')
