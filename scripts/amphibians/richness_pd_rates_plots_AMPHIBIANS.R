...
hex_grid <- readRDS('../objects/hex_grid_AMPHIBIANS.rds')
hex_grid0 <- hex_grid
hex_grid <- hex_grid %>%
  filter(spRichness > 1)
...
epm_richness_plot <- ggplot() +
  ...
  theme_minimal() +
  labs(title = 'AMPHIBIAN species richness') +
...
epm_logdr_plot <- ggplot() +
  ...
  theme_minimal() +
  labs(title = 'AMPHIBIAN log DR rate') +
  theme(legend.position = 'bottom', 
...
res_pd_richness_AMPHIBIANS_plot <- ggplot() +
  geom_sf(data = world_projected, col = 'transparent', fill = 'gray80') +
...
dr_respdrich_plot_AMPHIBIANS <- ggplot(data = hex_grid, 
...
# Save plots ----
ggsave(filename = 'plots/richness_AMPHIBIANS.pdf', 
       plot = epm_richness_plot)
ggsave(filename = 'plots/pd_AMPHIBIANS.pdf', 
       plot = epm_pd_plot)
ggsave(filename = 'plots/dr_AMPHIBIANS.pdf', 
       plot = epm_logdr_plot)
ggsave(filename = 'plots/pd_richness_AMPHIBIANS.pdf', 
       plot = scatterplot_residuals)
ggsave(filename = 'plots/res_AMPHIBIANS.pdf', 
       plot = res_pd_richness_AMPHIBIANS_plot)
ggsave(filename = 'plots/regression_dr_AMPHIBIANS.pdf', 
       plot = dr_respdrich_plot_AMPHIBIANS)
