...
# Purpose ----
# Make the object EPM for all amphibians 

# Date: 2022/11/03
setwd('/Volumes/GoogleDrive/.shortcut-targets-by-id/1UUotV2ZfjWpi2pbuYPKkz5zwS_tRU-IO/Hector_Jiri_PROJECTS/richness-pd-rates/richness_pd_dr')

output_path_to_save <- 'output/epm_output_amphibians.txt'
...
dist_file <- '../data/distribution/amphibian_dist_clean.rds'
...
#?createEPMgrid
amph_EPM <- createEPMgrid(spListEA, 
                           resolution = res, 
...
object_path_to_save <- '../objects/EPM_amph.rds'
saveRDS(amph_EPM, object_path_to_save)
