...
hex_grid <- readRDS('../objects/hex_grid_ter_BIRDS.rds')

# Projection
behrmann <- "+proj=cea +lon_0=0 +lat_ts=30 +x_0=0 +y_0=0 +datum=WGS84 +ellps=WGS84 +units=m +no_defs"
st_crs(hex_grid) <- behrmann # behrmann projection
hex_grid <- st_transform(hex_grid, crs = behrmann)
proj_hexgrid <- crs(hex_grid)
...
hex_grid_ter <- st_intersects(hex_grid, world_projected)
class(hex_grid_ter)
length(hex_grid_ter)
length()
hex_grid_ter.vec <- as.vector(hex_grid_ter)
hex_grid[hex_grid_ter.vec]
hex_grid_ter.vec <- unlist(hex_grid_ter)
...
epm_richness_plot <- ggplot() +
  ...
  geom_sf(data = hex_grid0, aes(fill = spRichness), col = 'transparent') +
...
vn <- length(unique(hex_grid0$spRichness))
...
plot(hex_grid, col = vir[hex_grid$spRichness], 
     border = NA, add = TRUE)
plot(hex_grid0[hex_grid0$spRichness < 2,], col = 'red', 
     border = NA, add = TRUE)
...
epm_pd_plot <- ggplot() +
  geom_sf(data = world_projected, col = 'transparent', fill = 'gray80') +
  geom_sf(data = hex_grid0, aes(fill = pd/1000), col = 'transparent') +
...
epm_logdr_plot <- ggplot() +
  geom_sf(data = world_projected, col = 'transparent', fill = 'gray80') +
  geom_sf(data = hex_grid0, aes(fill = logdr), col = 'transparent') +
  scale_fill_viridis_c(option = 'viridis', name = 'logDR') +
  theme_minimal() +
  labs(title = 'BIRD log DR rate') +
  theme(legend.position = 'bottom', 
...
epm_dr_plot <- ggplot() +
  geom_sf(data = world_projected, col = 'transparent', fill = 'gray80') +
  geom_sf(data = hex_grid0, aes(fill = dr), col = 'transparent') +
  scale_fill_viridis_c(option = 'viridis', name = 'DR') +
  theme_minimal() +
  labs(title = 'BIRD DR rate') +
  theme(legend.position = 'bottom', 
        plot.title = element_text(hjust = 0.5, 
                                  face = 'bold'))
...
hex_grid[which.min(hex_grid$resloess_pd_rich),]

hist(hex_grid$spRichness)
hist(hex_grid$resloess_pd_rich)
...
res_pd_richness_BIRDS_plot <- ggplot() +
  geom_sf(data = world_projected, col = 'transparent', fill = 'gray80') +
...
dr_respdrich_plot_BIRDS <- ggplot(data = hex_grid, 
...
# Save plots ----
ggsave(filename = 'plots/richness_BIRDS.pdf', 
       plot = epm_richness_plot)
ggsave(filename = 'plots/pd_BIRDS.pdf', 
       plot = epm_pd_plot)
ggsave(filename = 'plots/dr_BIRDS.pdf', 
       plot = epm_logdr_plot)
ggsave(filename = 'plots/pd_richness_BIRDS.pdf', 
       plot = scatterplot_residuals)
ggsave(filename = 'plots/res_BIRDS.pdf', 
       plot = res_pd_richness_BIRDS_plot)
ggsave(filename = 'plots/regression_dr_BIRDS.pdf', 
       plot = dr_respdrich_plot_BIRDS)
