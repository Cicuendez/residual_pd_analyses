# Purpose ----
# Get the mean phylogenetic diversity (PD) with 100 posterior bird trees
# 
# Date: 2022/11/03
wd <- '/Users/imac2016/Dropbox/hector_working/richness_pd_dr'
setwd(wd)
sink(file="output/get_meanPD_output_BIRDS.txt", 
     append=F, split=TRUE)
...
for (i in 1:length(postrees)){
     ...
     tree <- postrees[[i]]
     bird_EPM <- addPhylo(bird_EPM, tree, replace = TRUE)
     print("Phylogeny added to EPM object")

     print("running gridMetrics() to get PD...") 
     epm_pd <- gridMetrics(bird_EPM, metric = 'pd')
...
# Save pd_mean object ----
saveRDS(pd_mean, 'objects/pd_mean_BIRDS.rds')
