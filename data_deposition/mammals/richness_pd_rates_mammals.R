...
sprichness_grid <- mam_EPM$grid['spRichness']

# Add DR data ----
# Import DR data
dr_mean <- readRDS("../objects/dr_mean_MAMMALS.rds")
nrow(dr_mean)
head(dr_mean)
rownames(dr_mean)
...
mam_EPM <- addTraits(mam_EPM, dr_vector, replace = TRUE)
head(mam_EPM$data)

# Get DR standard deviation grid ----
epm_dr_sd <- gridMetrics(mam_EPM, metric = 'variance')
colnames(epm_dr_sd$grid)[colnames(epm_dr_sd$grid) == 'variance'] <- 'dr_sd'
dr_grid_sd <- epm_dr_sd$grid['dr_sd']

# Get DR grid ----
epm_dr <- gridMetrics(mam_EPM, metric = 'mean')
...
# Get PD grid ----
pd_mean <- readRDS('../objects/pd_mean_MAMMALS.rds')
