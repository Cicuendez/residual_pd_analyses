...
hex_grid <- readRDS('../objects/hex_grid_SQUAMATES.rds')
hex_grid0 <- hex_grid
hex_grid <- hex_grid %>%
  filter(spRichness > 1)
...
res_pd_richness_squam_plot <- ggplot() +
  geom_sf(data = world_projected, col = 'transparent', fill = 'gray80') +
...
dr_respdrich_plot_squam <- ggplot(data = hex_grid, 
...
# Save plots ----
ggsave(filename = 'plots/richness_squamates.pdf', 
       plot = epm_richness_plot)
ggsave(filename = 'plots/pd_squamates.pdf', 
       plot = epm_pd_plot)
ggsave(filename = 'plots/dr_squamates.pdf', 
       plot = epm_logdr_plot)
ggsave(filename = 'plots/pd_richness_squamates.pdf', 
       plot = scatterplot_residuals)
ggsave(filename = 'plots/res_squamates.pdf', 
       plot = res_pd_richness_squam_plot)
ggsave(filename = 'plots/regression_dr_squamates.pdf', 
       plot = dr_respdrich_plot_squam)
